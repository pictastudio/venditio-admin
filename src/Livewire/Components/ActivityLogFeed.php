...
namespace PictaStudio\VenditioAdmin\Livewire\Components;
...
use PictaStudio\VenditioAdmin\Facades\ActivityLog;
...
class ActivityLogFeed extends Component implements HasActions, HasForms
{
    ...
    public function activityLog(): \Illuminate\Pagination\LengthAwarePaginator
    {
        ...
        $activities->setCollection($activities->groupBy(fn ($log) => $log->created_at->format('Y-m-d'))
            ->map(function ($logs) {
                return [
                    ...
                    'items' => $logs->map(function ($log) {
                        return [
                            'log' => $log,
                            'renderers' => $this->renderers->filter(fn ($render) => $render['event'] == $log->event)->pluck('class'),
                        ];
                    }),
                ];
            }));
