...
class ProductResource extends Resource
{
    ...
    {
        return __('venditio-admin::global.sections.catalog');
    }
            ...
            ->schema([
                Tabs::make()
                    ...
                    ->schema([
                        Tab::make(__('venditio-admin::product.form.tabs.details.label'))
                            ...
                            ->schema([
                                Select::make('brand_id')
                                    ->label(__('venditio-admin::product.form.brand.label'))
                                    ->required()
                                    ->relationship('brand', 'name'),
                                Select::make('product_type_id')
                                    ->label(__('venditio-admin::product.form.product_type.label'))
                                    ->required()
                                    ->relationship('productType', 'name'),
                                Select::make('tax_class_id')
                                    ->label(__('venditio-admin::product.form.tax_class.label'))
                                    ->required()
                                    ->relationship('taxClass', 'name'),
                                Select::make('category')
                                    ->label(__('venditio-admin::product.form.category.label'))
                                    ->required()
                        ...
                        Tab::make(__('venditio-admin::product.form.tabs.dimensions.label'))
                            ...
                            ->schema([
                                Select::make('measuring_unit')
                                    ->label(__('venditio-admin::product.form.measuring_unit.label'))
                                    ->nullable()
                                    ->options([
                                        'pz' => 'pz',
                                    ]),
                        ...
                        Tab::make(__('venditio-admin::product.form.tabs.variants.label'))
                            ...
                            ->schema([
                                Repeater::make('variants')
                                    ->label(false)
                                    ->dehydrated(false)
                                    ->addActionLabel(__('venditio-admin::product.form.variants.add_button_label'))
                                    ->defaultItems(0)
                                    ->grid(2)
                                    ->schema([
        ...
        return $table
            ->columns([
                TextColumn::make('name')
                    ->label(__('venditio-admin::product.table.name.label'))
                    ->searchable(),
                IconColumn::make('active')
                    ->label(__('venditio-admin::product.table.active.label'))
